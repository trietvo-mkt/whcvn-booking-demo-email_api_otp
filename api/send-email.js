export default async function handler(req, res) {
  ...
  const { to, bookingId, employee, packageName } = req.body;

  // Validate required fields
  if (!to || !bookingId || !employee) {
    ...
    <div style="font-family: 'Segoe UI', Arial, sans-serif; max-width: 600px; margin: 0 auto; background: #ffffff;">
      ...
      <div style="background: #00adee; padding: 24px; text-align: center;">
        ...
        <table width="100%" cellpadding="0" cellspacing="0" border="0"><tr><td align="center">
          ...
          <p style="color: #ffffff; margin: 12px 0 0; font-size: 14px; opacity: 0.9;">Hệ thống đặt lịch dịch vụ</p>
        </td></tr></table>
      </div>
      ...
      <div style="text-align: center; padding: 32px 24px 16px;">
        <table width="100%" cellpadding="0" cellspacing="0" border="0"><tr><td align="center">
          <table cellpadding="0" cellspacing="0" border="0"><tr>
            <td width="64" height="64" align="center" valign="middle" style="border-radius: 50%; background-color: #00adee; text-align: center; vertical-align: middle;">
              <img src="data:image/svg+xml;base64," alt="✓" width="32" height="32" style="display: block;" />
            </td>
          </tr></table>
        </td></tr></table>
        <h2 style="color: #00adee; margin: 16px 0 4px; font-size: 22px;">Đặt lịch hẹn thành công!</h2>
        <p style="color: #64748B; margin: 0; font-size: 14px;">Mã lịch hẹn <strong style="color: #1E293B;">${bookingId}</strong></p>
      </div>

      <!-- Booking Info Card -->
      <div style="margin: 16px 24px; border: 1px solid #E2E8F0; border-radius: 12px; overflow: hidden;">
        <!-- Thông tin cá nhân -->
        <div style="border-left: 3px solid #00adee; padding: 12px 16px; background: #F8FAFC;">
          <strong style="font-size: 14px; color: #1E293B;">Thông tin cá nhân</strong>
        </div>
        <div style="padding: 16px;">
          <table style="width: 100%; font-size: 14px; color: #1E293B;">
            ${packageName ? `<tr><td style="color: #64748B; padding: 4px 0;">Gói đăng ký:</td><td style="font-weight: 600; padding: 4px 0;">${packageName}</td></tr>` : ''}
            <tr><td style="color: #64748B; padding: 4px 0;">Họ Tên:</td><td style="font-weight: 600; padding: 4px 0;">${employee.name}</td></tr>
            <tr><td style="color: #64748B; padding: 4px 0;">Mã nhân viên:</td><td style="font-weight: 600; padding: 4px 0;">${employee.employeeId}</td></tr>
            <tr><td style="color: #64748B; padding: 4px 0;">Email:</td><td style="font-weight: 600; padding: 4px 0;">${employee.email}</td></tr>
            <tr><td style="color: #64748B; padding: 4px 0;">Ngày sinh:</td><td style="font-weight: 600; padding: 4px 0;">${employee.dob}</td></tr>
            <tr><td style="color: #64748B; padding: 4px 0;">Giới tính:</td><td style="font-weight: 600; padding: 4px 0;">${employee.gender}</td></tr>
          </table>
        </div>

        <!-- Thông tin lịch khám -->
        <div style="border-left: 3px solid #00adee; padding: 12px 16px; background: #F8FAFC; border-top: 1px solid #E2E8F0;">
          <strong style="font-size: 14px; color: #1E293B;">Thông tin lịch khám</strong>
        </div>
        <div style="padding: 16px;">
          <table style="width: 100%; font-size: 14px; color: #1E293B;">
            <tr><td style="color: #64748B; padding: 4px 0;">Tỉnh/Thành phố:</td><td style="font-weight: 600; padding: 4px 0;">${employee.city}</td></tr>
            <tr><td style="color: #64748B; padding: 4px 0;">Bệnh viện/Phòng khám:</td><td style="font-weight: 600; padding: 4px 0;">${employee.hospital}</td></tr>
            <tr><td style="color: #64748B; padding: 4px 0;">Ngày hẹn:</td><td style="font-weight: 600; padding: 4px 0;">${employee.date}</td></tr>
            <tr><td style="color: #64748B; padding: 4px 0;">Giờ khám:</td><td style="font-weight: 600; padding: 4px 0;">${employee.time}</td></tr>
          </table>
        </div>
      </div>

      <!-- Footer -->
      <div style="padding: 24px; text-align: center; border-top: 1px solid #E2E8F0; margin-top: 16px;">
        <p style="color: #64748B; font-size: 13px; margin: 0 0 4px;">Liên hệ hỗ trợ: <strong style="color: #1E293B;">1900 5118</strong> | Email <strong style="color: #1E293B;">info@whitecoat.vn</strong></p>
        <p style="color: #64748B; font-size: 12px; margin: 0;">CÔNG TY TNHH WHITECOAT VIỆT NAM</p>
      </div>
    </div>
  `;
  ...
  try {
    const response = await fetch('https://api.brevo.com/v3/smtp/email', {
      ...
      body: JSON.stringify({
        ...
        subject: `Xác nhận đặt lịch khám sức khỏe - ${bookingId}`,
